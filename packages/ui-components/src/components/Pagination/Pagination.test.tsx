...
describe('Pagination', () => {
  ...
  it('forwards className to the nav element', () => {
    render(
      <Pagination page={1} totalPages={5} onPageChange={() => {}} className="custom-class" />
    );
  ...
  describe('disabled', () => {
    it('disables previous, next, and page buttons when disabled is true', () => {
      render(<Pagination page={3} totalPages={5} onPageChange={() => {}} disabled />);
      expect(screen.getByRole('button', { name: 'Página anterior' })).toBeDisabled();
      expect(screen.getByRole('button', { name: 'Página siguiente' })).toBeDisabled();
    ...
    it('clicking a page button while disabled does not call onPageChange', () => {
      const onPageChange = vi.fn();
      render(<Pagination page={3} totalPages={5} onPageChange={onPageChange} disabled />);
      fireEvent.click(screen.getByRole('button', { name: 'Página 2' }));
      expect(onPageChange).not.toHaveBeenCalled();
    });

    it('buttons are enabled when disabled is false (default)', () => {
      render(<Pagination page={3} totalPages={5} onPageChange={() => {}} />);
      expect(screen.getByRole('button', { name: 'Página 3' })).not.toBeDisabled();
    });
  });
});
